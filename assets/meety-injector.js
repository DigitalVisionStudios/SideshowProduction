...
(function(){
  ...
  function isCartAdd(url, method){
    if (!url) return false; const m = String(method||'GET').toUpperCase(); if (m !== 'POST') return false; return /\/cart\/(add|add\.js)(\?|$)/i.test(url);
  }
  function ensureContextFresh(){ const ctx = window.__meetyBogoContext; if (!ctx) return null; if (Date.now() - (ctx.at||0) > 10*60*1000) return null; return ctx; }
  ...
  function injectBogoPropsIntoFormData(fd, ctx){ if(!fd||!ctx) return fd; try { fd.set('properties[BOGO Charged Service]', ctx.charged.title); fd.set('properties[BOGO Free Service]', ctx.free.title+' (FREE)'); if(ctx.free2) fd.set('properties[BOGO Free Service 2]', ctx.free2.title+' (FREE)'); fd.set('properties[BOGO Total Savings]', `$${(ctx.savingsCents/100).toFixed(2)}`); if(ctx.category) fd.set('properties[BOGO Category]', ctx.category); ctx.injected=true; console.log(TAG,'Injected BOGO props (FormData) multiFree?', !!ctx.free2); } catch(e){ console.warn(TAG,'FormData inject failed', e); } return fd; }
  function injectBogoPropsIntoJson(obj, ctx){
    ...
    const add=(t)=>{
      ...
      const base={
        ...
        'BOGO Free Service': ctx.free.title+' (FREE)',
  ...
  function transformBody(body){ const ctx=ensureContextFresh(); const cfg=window.__meetyDownpayConfig && window.__meetyDownpayConfig.enabled ? window.__meetyDownpayConfig : null; try { if(body instanceof FormData){ if(ctx) injectBogoPropsIntoFormData(body, ctx); if(cfg){ refreshTotal(); const t=cfg.lastTotal; if(t){ const split=computeSplit(t.cents, cfg); body.set(`properties[${cfg.baseLabel}]`, `${t.symbol}${(split.baseCents/100).toFixed(2)}`); body.set(`properties[${cfg.feeLabel}]`, `${t.symbol}${(split.feeCents/100).toFixed(2)}`); } } return body; } if(typeof URLSearchParams!=='undefined' && body instanceof URLSearchParams){ if(ctx){ body.set('properties[BOGO Charged Service]', ctx.charged.title); body.set('properties[BOGO Free Service]', ctx.free.title+' (FREE)'); if(ctx.free2) body.set('properties[BOGO Free Service 2]', ctx.free2.title+' (FREE)'); body.set('properties[BOGO Total Savings]', `$${(ctx.savingsCents/100).toFixed(2)}`); if(ctx.category) body.set('properties[BOGO Category]', ctx.category); ctx.injected=true; console.log(TAG,'Injected BOGO props (URLSearchParams) multiFree?', !!ctx.free2); } if(cfg){ refreshTotal(); const t=cfg.lastTotal; if(t){ const split=computeSplit(t.cents, cfg); body.set(`properties[${cfg.baseLabel}]`, `${t.symbol}${(split.baseCents/100).toFixed(2)}`); body.set(`properties[${cfg.feeLabel}]`, `${t.symbol}${(split.feeCents/100).toFixed(2)}`); } } return body; } if(typeof body==='string'){ const s=body.trim(); const isJSON=(s.startsWith('{')&&s.endsWith('}'))||(s.startsWith('[')&&s.endsWith(']')); if(isJSON){ const parsed=JSON.parse(s||'{}'); if(ctx) injectBogoPropsIntoJson(parsed, ctx); if(cfg){ if(Array.isArray(parsed.items)) parsed.items.forEach(it=>addDownpayProps(it, cfg)); else addDownpayProps(parsed, cfg); } return JSON.stringify(parsed); } else { const usp=new URLSearchParams(s); transformBody(usp); return usp.toString(); } } if(body && typeof body==='object' && !(body instanceof Blob)){ if(ctx) injectBogoPropsIntoJson(body, ctx); if(cfg){ if(Array.isArray(body.items)) body.items.forEach(it=>addDownpayProps(it, cfg)); else addDownpayProps(body, cfg); } return body; } } catch(e){ console.warn(TAG,'transformBody error', e); } return body; }
